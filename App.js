...
import AddPhone from './components/Todo'
  ...
  renderQrCode = (user) => {
    return (
      <ScrollView style={{ flex: 10 }}>
        ...
        <View style={{ flex: 3 }}>
          <AddPhone ref="child" customerPhoneNumber={user["phoneNumber"]} confirm={this.state.resualt} fullName={this.state.fullName} />
        ...
        <Singup signout={() => this.signOut()}
          qrcode={() => this.showBarCode()}
      ...
      <View style={{ flex: 1 }}>
       {!user && !confirmResult && this.renderPhoneNumberInput()}
        ...
        {user && this.state.username && (
          <View style={styles.container}>
            <Qrcode customerPhoneNumber={user["phoneNumber"]} />
            <Button title="Back" onPress={() => this.setState({ username: false })} />
          </View>
        )
        }
      </View>
